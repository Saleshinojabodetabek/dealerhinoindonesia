...
// Daftar grup spesifikasi (sama seperti di tambah_produk.php)
$spec_groups = [
...
// Ambil data produk
$result = $conn->query("SELECT * FROM produk WHERE id = $produk_id");
...
while ($row = $res_spec->fetch_assoc()) {
    $specData[$row['grup']][] = ['label'=>$row['label'],'value'=>$row['nilai']];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update produk
    $sql = "UPDATE produk SET series_id='$series_id', varian='$varian', nama_produk='$nama_produk', deskripsi='$deskripsi', gambar='$gambar' WHERE id=$produk_id";
    if (!$conn->query($sql)) {
...
<?php
$varianOptions = ['All','Cargo','Dump','Mixer'];
...
<img src="../uploads/karoseri/<?= htmlspecialchars($kr['slug']) ?>.png" style="width:50px;height:auto;object-fit:contain;" class="me-2 border rounded">
